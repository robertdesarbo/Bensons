...
@Component({
  selector: 'dialog-field-location-dialog',
...
export class DialogFieldLocation {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    if (this.injectedData.type === FormActionList.remove || this.injectedData.type === FormActionList.edit) {
      ...
      this.fieldLocation$.subscribe((fieldLocation: FieldLocation) => {
        ...
        const fields = this.formControl.get('fields') as FormArray;
        fieldLocation.fields.forEach((field: Field) => {
          fields.push(this.createFieldFormGroup(field));
        });
  ...
  private createFieldFormGroup(field: Field): FormGroup {
    return new FormGroup({
      number: new FormControl(field?.number, Validators.required),
      name: new FormControl(field?.name, Validators.required),
      sport: new FormControl(field?.sport, Validators.required),
      lights: new FormControl(field?.lights, Validators.required),
      active: new FormControl(field?.active, Validators.required),
      alcohol: new FormControl(field?.alcohol),
      privateProperty: new FormControl(field?.private_property),
      pets: new FormControl(field?.pets),
      smoking: new FormControl(field?.smoking),
      groundRules: new FormControl(field?.ground_rules),
    });
  }
